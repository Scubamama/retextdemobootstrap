...
/**
 * CRUD functionality for the book titles table Uses prepared statements to
 * access a database
 * 
...
public class BookTitlesDAO {
	...
	public List<BookTitles> searchBooks(String text) throws SQLException {
		...
		try {
			...
			while (myRs.next()) {
				bookList.add(new BookTitles(myRs.getInt("Id"), myRs.getString("Title"), myRs.getString("Author"),
...
//				}
	...
	public BookTitles get(Integer id) throws SQLException {
		...
		try {
			...
			if (myRs.next()) {
				BookTitles b = new BookTitles(myRs.getInt("Id"), myRs.getString("Title"), myRs.getString("Author"),
						myRs.getString("Edition"), myRs.getString("Isbn"));
				return b;
